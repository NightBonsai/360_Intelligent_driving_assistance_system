...
void IPC::semP(int semrsvIndex)
{
	struct sembuf buf = { semrsvIndex,-1,SEM_UNDO };
...
void IPC::setShm(char resbuf[])
{
	...
	semP(this->sndIndex);
		...
		shmArr[sndIndex] = 1;
		memcpy(shmAddr + sizeof(int) * sndIndex, &shmArr[rsvIndex], sizeof(int));						//操作索引区：首地址 + 一个索引大小*下标
		cout << "后置服务器业务返回包丢进共享内存 " << endl;
		...
		LOGINBACK body = { 0 };
		memcpy(&body, data + sizeof(HEAD), head.length);
		cout << "测试读取业务包体" << endl;
...
bool IPC::listCompare(PHOTO_INFO a, PHOTO_INFO b)
{
	//升序排列
	if (a.p_filePageIndex < b.p_filePageIndex)
	{
		return true;
	}
	else
