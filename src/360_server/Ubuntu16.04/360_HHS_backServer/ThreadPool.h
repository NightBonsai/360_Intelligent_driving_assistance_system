...
#define MIN_NUM 10
#define LOG_DERIVE_TIME 300	//日志导出间断时间；单位：秒

using namespace std;

//线程池类：从共享内存接收客户端发来的业务包
class ThreadPool
{
public:
	...
	static void* logDeriveThreadFun(void* arg);		//日志表导出线程执行函数：一个时间段后导出日志表
private:
	...
	pthread_t logDeriveThread;		//日志表导出线程ID	
};
